...
- **AI 端点**（如 `POST /ai/chat`、`POST /diet/records` 含 AI 解析、`POST /plans` 含 AI 创建）：API 层调用 Agent，Agent 通过 Tool 调用 Service。
- **纯 CRUD 端点**（如 `GET /users/me`、`PUT /users/me/profile`、`GET /diet/records`、`GET /diet/daily-summary`、`POST /body/weight`、`GET /plans/{id}/progress`）：API 层直接调用 Service，不经过 Agent。
- Service 层 → Repository 层、外部集成层（Embedding/pgvector）
- Service 层之间可互相调用（通过依赖注入）
- **禁止** 任何层直接调用 LLM（必须走 Agent）
...
| 饮食 | `POST /diet/records`（含 AI 解析）、`POST /diet/parse` | `GET/PUT/DELETE /diet/records/*`、`GET /diet/daily-summary`、`GET /diet/weekly-summary` |
| 身体数据 | — | 所有 `POST/GET/PUT/DELETE /body/*` |
| 计划 | `POST /plans`（AI 对话创建） | `GET/PUT/DELETE /plans/*`、`POST /plans/{id}/check-ins`、`GET /plans/{id}/progress` |
| AI 记忆 | `POST /ai/chat` | `GET /ai/chat/history`、`DELETE /ai/chat/sessions/{id}` |
