...
│  LLM 推理的唯一入口。Agent 通过 Tool 调 Service  │
│  diet_agent / chat_agent / plan_agent /       │
│  memory_agent / suggestion_agent              │
├─────────────────────────────────────────────┤
│             Service 层 (业务逻辑)              │
│  DB CRUD 编排、营养/BMR 等纯算法、非 AI 业务     │
│  作为 Agent 的 Tool 实现，不做 LLM 编排          │
├─────────────────────────────────────────────┤
│           Repository 层 (数据访问)             │
│  数据库 CRUD、查询构建、事务管理                │
├──────────────┬──────────┬───────────────────┤
...
| 层级 | 职责 | 禁止 |
|------|------|------|
| API 层 | 路由注册、参数校验、响应包装、认证检查 | 业务逻辑、直接调用 Repository |
| Agent 层 | LLM 推理编排、Tool 调度、多步流程管理 | DB 直连；绕过 Service 做 CRUD |
| Service 层 | DB CRUD 编排、纯算法逻辑（BMR、营养计算等） | LLM 调用、LLM 场景方法封装 |
| Repository 层 | SQL 查询、数据映射、事务管理 | 业务判断、跨 repo 调用 |
| 外部集成层 | Embedding、pgvector、Supabase Auth | 感知业务上下文 |

### 3.2 调用规则

- API 层 → Agent 层（AI 业务）或 Service 层（纯 CRUD 业务，如用户档案）
- Agent 层 → 通过 Tool 调用 Service 层
